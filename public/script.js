// Updated script.js with improved URL checker handling
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function getBotResponse(message) {
    try {
      ...
      return '⚠️ Sorry, something went wrong with the chat service.';
    }
  }
  ...
  urlCheckerForm.addEventListener('submit', async (e) => {
    ...
    if (!url) {
      urlResult.innerHTML = '<span class="text-red-600">Please enter a URL.</span>';
      return;
    }
    ...
    try { 
      ...
      urlResult.innerHTML = '<span class="text-red-600">Invalid URL format. Please include http:// or https://</span>';
      return;
    }
    
    // Show loading state
    urlResult.innerHTML = '<span class="text-blue-600">🔍 Checking URL security...</span>';
    const submitButton = urlCheckerForm.querySelector('button');
    const originalButtonText = submitButton.textContent;
    submitButton.disabled = true;
    submitButton.textContent = 'Checking...';
    
    try {
      console.log('Sending URL check request for:', url);
      ...
      console.log('URL check response status:', res.status);
      ...
      // Create detailed message based on verdict
      let messageHtml = '';
      let messageClass = '';
      ...
      switch (result.verdict) {
        case 'malicious':
          ...
          messageHtml = `🚨 <strong>DANGER:</strong> This URL is flagged as malicious!<br>
                        <small class="text-red-500">${sanitize(result.explanation || 'Avoid visiting this link.')}</small>`;
          break;
          
        case 'suspicious':
          messageClass = 'text-orange-600 font-bold';
          messageHtml = `⚠️ <strong>SUSPICIOUS:</strong> This URL has concerning characteristics.<br>
                        <small class="text-orange-500">${sanitize(result.explanation || 'Proceed with caution.')}</small>`;
          break;
          
        case 'safe':
          messageClass = 'text-green-600 font-bold';
          messageHtml = `✅ <strong>SAFE:</strong> This URL appears to be legitimate.<br>
                        <small class="text-green-500">${sanitize(result.explanation || 'No known security threats detected.')}</small>`;
          break;
          
        case 'unknown':
        default:
          messageClass = 'text-gray-600';
          messageHtml = `❓ <strong>UNKNOWN:</strong> Could not determine URL safety.<br>
                        <small class="text-gray-500">${sanitize(result.explanation || 'Exercise normal caution when visiting.')}</small>`;
          break;
      }
      
      // Add fallback indicator if applicable
      if (result.fallback) {
        messageHtml += '<br><small class="text-blue-500">⚠️ Using fallback security check</small>';
      }
      
      // Display result in URL result area
      urlResult.innerHTML = `<div class="${messageClass}">${messageHtml}</div>`;
      
      // Also add to chat for better visibility
      const chatMessage = `🔗 <strong>URL Security Check:</strong><br>
                          <strong>URL:</strong> ${sanitize(url)}<br>
      ...
      const errorMessage = `⚠️ <strong>Error:</strong> ${sanitize(error.message)}`;
      urlResult.innerHTML = `<div class="text-red-600">${errorMessage}</div>`;
      
      appendMessage('bot', `🔗 <strong>URL Check Failed:</strong><br>
                           <strong>URL:</strong> ${sanitize(url)}<br>
                           <div class="text-red-600">${errorMessage}<br>
                           <small>Please try again later or verify the URL manually.</small></div>`);
    } finally {
      // Reset button state
  ...
  // --- Quiz Modules (keeping existing code) ---
  const quizModules = {
    phishing: {
      title: "Phishing",
